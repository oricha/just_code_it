...
//In other words, if a query is executed second, then its waiting time is the duration of the first
//query; if a query is executed third, then its waiting time is the sum of the durations of the first two queries.
//
...
//executed in the order of [5, 1, 4] would be (0) + (5) + (5 + 1) = 11. The first query of duration 5
//would be executed immediately, so its waiting time would be 0, the second query of duration 1 would have
//to wait 5 seconds (the duration of the first query) to be executed, and the last query would have to wait the
