package com.algorithms.encryption;
...
public class Hemming {
	public byte[] encode(String s) {
		...
		byte[] control = new byte[info.length / 2]; // 5 new control bits (1
													// byte) per each 16
													// informational bits
	...
	private static void print(byte[] info, byte[] control) {
		for (int i = 0, p = 0; i < info.length; i += 2, p++) {
			System.out.printf("%d, %d; %s+%s (%s)\n", info[i], info[i + 1], printByte(info[i]), printByte(info[i + 1]),
					printByte(control[p]));
		}
	}
